...
function WalletsDialogue({
  ...
  const filteredData = useCallback(() => {
    if (!isMobile) {
      return walletsItems.filter(({ name }) => name.toUpperCase().includes(search));
    } else {
      if (window.ethereum) {
        return [
          {
        ...
        return mobileWalletList.filter(({ name }) => name.toUpperCase().includes(search));
      }
    }
  }, [search, walletList]);
  ...
  const addressShorten = address => {
    if (address) {
      return `${address.slice(0, 6)}...${address.slice(address.length - 2, address.length)}`;
    }
  };
